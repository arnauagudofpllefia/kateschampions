    ...
    <main className="pro-shell">
      <section className="hero-panel home-hero-image rounded-3xl border border-white/20 p-6 sm:p-10">
        ...
        <div className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <article className="inner-panel p-4">
            <p className="text-xs uppercase text-slate-300">Equipos</p>
            <p className="kpi-value mt-1 text-slate-100">{resumen.totalEquipos}</p>
          </article>
          <article className="inner-panel p-4">
            <p className="text-xs uppercase text-slate-300">Ultimos resultados</p>
            <p className="kpi-value mt-1 text-slate-100">
              {resumen.totalPartidosJugados}
            </p>
          </article>
          <article className="inner-panel p-4">
            <p className="text-xs uppercase text-slate-300">Proximos</p>
            <p className="kpi-value mt-1 text-slate-100">
              {resumen.totalPartidosProximos}
            </p>
          </article>
          <article className="inner-panel p-4">
            <p className="text-xs uppercase text-slate-300">Lider actual</p>
            {resumen.top3[0] ? (
              <p className="mt-1 text-lg font-black text-[#f8d66d]">
      ...
      <section className="mt-8 grid gap-6 lg:grid-cols-2">
        <article className="section-card p-5">
          <h2 className="text-2xl font-black text-slate-100">Top 3 clasificacion</h2>
          <ul className="mt-4 space-y-3">
            {resumen.top3.map((team, index) => (
              <li key={team.id} className="inner-panel flex items-center justify-between p-3 text-slate-100">
                ...
                <span className="font-bold text-[#f8d66d]">{team.points} pts</span>
              </li>
            ))}
          </ul>
        </article>
        ...
        <article className="section-card p-5">
          <h2 className="text-2xl font-black text-slate-100">Partidos por dia</h2>
          <ul className="mt-4 space-y-3 text-sm text-slate-200">
            {dias.map((dia) => (
              <li key={dia} className="inner-panel p-3">
                <p className="font-semibold text-[#f8d66d]">{dia}</p>
